...
description: Run the JobFinder LinkedIn job-search-and-apply workflow scoped to Europe (and Japan)-based/remote roles - search LinkedIn, judge fit against whatever resume is configured, and auto-apply via Easy Apply when the match is good and all screening questions can be answered. Use when the user asks to run/check Europe job search, find new Europe jobs, or apply to Europe/Japan jobs today.
---

# Apply Jobs - Europe

This skill is the Europe-only counterpart of the JobFinder pipeline (see also `apply-jobs-india`).
It always searches with `config/config.europe.json` and always uses `--region eu` -- there is no
per-job region decision here, unlike the original combined skill. Its search locations also include
Japan-based roles, which use the same EU config/resume/answers per prior convention. It applies
...
The resume is not hardcoded — `config.europe.json`'s `resume` field points at whatever profile is
...
1. `config/config.europe.json` must exist — if missing, tell the user to copy
   `config/config.europe.json.example` to `config/config.europe.json`, fill in locations, resume
...
2. The file at `config.europe.json`'s `resume` path must exist — if missing or the user says it's
...
1. **Read the resume once.** Read `config.europe.json`'s `resume` file via the `Read` tool — reuse
   ...
   `config.europe.json`'s static `search.titles` list for this run — that list is only a fallback
...
3. **Search.** Run:
   ```
   python scripts/linkedin.py search --config config/config.europe.json --seen data/seen_jobs.json \
...
4. **For each job returned, in order:**
   ...
   c. **Match >= threshold:** run:
      ```
      python scripts/linkedin.py apply --job-id <id> --job-url <url> --resume-path <config.europe.json's resume path> \
...
5. **Handle apply events as they arrive:**
   ...
   - `question_pending` (`question`, `field_type`, `options`, `required`) — this is a screening
     ...
     **generic and reusable** (salary expectation, notice period, visa/work permit, relocation,
     gender/EEO, phone, location, years of experience, GDPR/privacy consent, Japanese-language
     screening questions, etc.) versus **company-specific** (e.g. "why do you want to join Acme?")
     or **bucketed/company-tailored option sets** unlikely to recur verbatim (e.g. "5+ years / 3-5
     years / ..." for a specific tech stack) — the latter two should just be answered live, not
     persisted. If genuinely generic, append a new entry to `data/answers.json` (same shape as
     existing entries: `category`, `match_substrings`, `field_type`, `value` or `value_by_region`,
     and optionally `regions: ["eu"]` if the answer is EUR-currency/context-specific and would be
     wrong if reused for an India application) so future runs match it automatically without
     asking again.
...
- Never search India locations or use the India resume/config from this skill — that's what
...
- If the resume changes between runs (new role, different seniority, career pivot), search titles
  ...
  new resume file, update `config.europe.json`'s `resume` field to point at it.
