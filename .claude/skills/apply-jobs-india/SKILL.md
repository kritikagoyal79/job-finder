...
description: Run the JobFinder LinkedIn job-search-and-apply workflow scoped to India-based/remote roles - search LinkedIn, judge fit against the India resume, and auto-apply via Easy Apply when the match is good and all screening questions can be answered. Use when the user asks to run/check India job search, find new India jobs, or apply to India jobs today.
---

# Apply Jobs - India

This skill is the India-only counterpart of the JobFinder pipeline (see also `apply-jobs-europe`).
It always searches with `config/config.india.json` and always uses the India resume and `--region
in` -- there is no per-job region decision here, unlike the original combined skill. It applies
...
1. `config/config.india.json` must exist — if missing, tell the user to copy
   `config/config.india.json.example` to `config/config.india.json`, fill in search
...
2. **Search.** Read `config/config.india.json` for `search.titles`, `search.locations`, and
   `chrome.profileDir`, then run:
   ```
   python scripts/linkedin.py search --config config/config.india.json --seen data/seen_jobs.json --run-timestamp <timestamp>
   ```
...
3. **For each job returned, in order:**
   a. Read the resume at `resume/Kritika_Goyal_Resume_IN.pdf` via the `Read` tool (once per run,
   ...
   c. **Match >= threshold:** run:
      ```
      python scripts/linkedin.py apply --job-id <id> --job-url <url> --resume-path resume/Kritika_Goyal_Resume_IN.pdf \
...
4. **Handle apply events as they arrive:**
   ...
   - `question_pending` (`question`, `field_type`, `options`, `required`) — this is a screening
     ...
     entries: `category`, `match_substrings`, `field_type`, `value` or `value_by_region`, and
     optionally `regions: ["in"]` if the answer is India-currency/context-specific and would be
     wrong if reused for a Europe application) so future runs match it automatically without
     asking again. Company-specific answers are used once and not persisted.
...
- Never search Europe/Japan locations or use the EU resume from this skill — that's what
